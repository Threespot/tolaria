...
class ManagedClassesTest < ActiveSupport::TestCase
  ...
  test "can manage, check, and unmanage a model" do
    ...
    assert managed_class.allows?(:index)
    assert managed_class.allows?(:show)
    refute managed_class.allows?(:edit)
    refute managed_class.allows?(:discard)
    ...
    assert Tolaria.managed_classes.collect(&:klass).exclude?(Card)
